 ...
 */
public abstract class PagerAdapterCopy {
    ...
    /**
     * 需要无限循环的adapter使用该方法替代getCount方法，不能重写getCount方法
     */
    public abstract int getRealCount();

    /**
     * 根据扩展position获取实际的position
     */
    public int getRealPosition(int position) {
        return getRealCount() > 0 ? position % getRealCount() : position;
    }

    /***
     * 在换页的时候调用这个，刷新页面的状态(有点类似RecyclerView的bindViewHolder()方法的作用)
     * @param extendPosition：扩展后的位置
     * @param realPosition：实际的位置
     */
    public abstract void onPageSelected(int extendPosition, int realPosition);
